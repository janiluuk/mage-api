...
class VideoJobOperationsController extends Controller
{
    ...
    public function extend(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $newJob->filename = 'extension_' . $baseJob->id . '_' . uniqid() . '.mp4';
            $newJob->original_filename = $baseJob->original_filename ?? 'extended_video.mp4';
            $newJob->outfile = 'out_extension_' . $baseJob->id . '_' . uniqid() . '.mp4';
